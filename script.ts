let FirstPlayer: number = 13;
let SecondPlayer: number = 24;

let RandomNumb: number = Math.floor(Math.random() * (100 - 1) + 1);
...
function checkNumb() {
    ...
    if (SecondPlayer === RandomNumb){
        ...
        if (Math.abs(RandomNumb - FirstPlayer) < Math.abs(RandomNumb - SecondPlayer)){
